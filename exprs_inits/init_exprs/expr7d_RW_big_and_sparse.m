...
vast3d_path = '/Users/matthewmerris/datasets/FROSTT/vast_2015_mini/vast-2015-mc1-3d.tns';
nell2_path = '/Users/matthewmerris/datasets/FROSTT/nell2/nell-2.tns';
...
dataset_names = ["vast3d","nell2"];
dataset_paths = {vast3d_path, nell2_path};
ranks = [10,9];
...
decomps_gcp = cell(num_tensors, num_runs, num_inits,3);
% decomps_als = cell(num_tensors, num_runs, num_inits,3);
for jdx = 1:num_tensors
    disp(dataset_names(jdx))
    nc = ranks(jdx);
    tns = data_tns{jdx};
    modes = ndims(tns);
    l_type = 'normal';
    if strcmp(dataset_names(kdx),"vast5d")
        l_type = 'binary';
    elseif strcmp(dataset_names(kdx),"nell2")
        l_type = 'count';
    end
    for idx = 1:num_runs
...
%         end
        ...
        if idx == 1
            [decomps_gcp{jdx,idx,1,1},decomps_gcp{jdx,idx,1,2},decomps_gcp{jdx,idx,1,3}] ... 
                = gcp_opt(tns, nc, 'type', l_type, ...
                'maxfails', 2, 'maxiters', max_iters, ...
                'printitn', 0, 'init', init_rand);
            [decomps_gcp{jdx,idx,2,1},decomps_gcp{jdx,idx,2,2},decomps_gcp{jdx,idx,2,3}] ... 
                = gcp_opt(tns, nc, 'type', l_type, ...
                'maxfails', 2, 'maxiters', max_iters, ...
                'printitn', 0, 'init', init_arnoldi);
...
%% save results
results_filename = sprintf('results/expr7d_RW_big_and_sparse_%dtensor_%dinits_%druns_cp_opt_', num_tensors, num_inits, ...
    num_runs)+ string(datetime("now"));
save(results_filename, 'szs', 'ranks','num_runs', 'tol', 'max_iters', 'num_tensors', 'num_inits', ...
    'decomps_gcp', 'init_times', 'inits', "dataset_names");
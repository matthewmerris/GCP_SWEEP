...
factor_match_score_gcp_opt = score(M_gcp, M_true, 'greedy', true)
fit_gcp = 1 - norm(X-full(M_gcp))/norm(X);
fprintf('Final fit: %e \n\n', fit_gcp);

tic, [M_als, M_0_als, out_cp] = cp_opt(X, R, 'init', M_init); 
cp_time = toc;
factor_match_score_cp_opt = score(M_als, M_true, 'greedy', true)
...
%%
figure
plot(factor_match_score_gcp_opt, out_gcp.mainTime, '+')
hold on
plot(factor_match_score_cp_opt, cp_time, 'ro')
plot(factor_match_score_apr, out_apr.ttlTime, 'go')
hold off
...
%%
figure
scatter3(factor_match_score_gcp_opt, fit_gcp, out_gcp.mainTime, '+')
hold on 
scatter3(factor_match_score_cp_opt, fit_cp, cp_time, 'ro')
scatter3(factor_match_score_apr, fit_apr, out_apr.ttlTime, 'go')
hold off
